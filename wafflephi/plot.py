...
def trend(
    df: pd.DataFrame, label: Optional[str] = None, stdev: Optional[bool] = True
    ...
    ax.yaxis.set_major_formatter(
        matplotlib.ticker.FuncFormatter(lambda y, _: "{:g}".format(10**y))
    )

    plt.xticks(
        range(0, df.shape[0], DATES_INTERVAL),
        df["Date"].loc[::DATES_INTERVAL],
        rotation=45,
        fontsize=20,
    )
...
def plot_wieght(df: pd.DataFrame, conv_model: "tf.keras.Model") -> None:
    train_df, val_df, test_df, train_mean, train_std = split_and_normalize(df)
    plt.bar(
        x=range(len(train_df.columns)), height=conv_model.layers[0].kernel[:, 0].numpy()
    )
    ax = plt.gca()
